...
export function InvitationViewPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-100 to-yellow-100 px-4 sm:px-6 py-8">
      ...
      {/* ホームへ戻るボタン - デスクトップではより広いコンテナ */}
      <div className="max-w-3xl lg:max-w-6xl mx-auto mb-4">
        <button
          onClick={() => navigate('/')}
          className="flex items-center text-indigo-800 text-lg focus:outline-none hover:text-indigo-600 transition-colors"
        >
          &larr; ホームへ
        </button>
      </div>
      ...
      {/* デスクトップでは2カラムレイアウト */}
      <div className="lg:grid lg:grid-cols-12 lg:gap-8 w-full max-w-3xl lg:max-w-6xl mx-auto">
        {/* メインコンテンツ - デスクトップでは8カラム */}
        <div className="lg:col-span-8">
          <motion.div
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="w-full bg-white border border-gray-200 rounded-2xl shadow-md hover:shadow-xl p-6 sm:p-8 mb-8"
          >
            {/* タイトルと日付 */}
            ...
            {/* クイックナビゲーション - デスクトップではより整理されたレイアウト */}
            <div className="flex flex-col sm:flex-row items-center justify-center flex-wrap gap-4 mb-8 border-b-2 border-purple-200 pb-8">
              <a 
                href="#members" 
                onClick={(e) => {
                  e.preventDefault();
            ...
            {/* 立替登録セクション - デスクトップで2カラムレイアウト */}
            <div id="expenses" className="space-y-6 mt-16">
              <h2 className="text-3xl font-semibold text-purple-800 text-center">
                支払いを記録する
              </h2>
              <div className="bg-gray-50 border border-gray-200 rounded-2xl shadow-md hover:shadow-xl transition-shadow p-8 space-y-6">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-2 rounded-md hover:from-purple-600 hover:to-pink-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                  {isLoading ? '追加中...' : '取引を追加する'}
                </button>
              </div>

              {/* 取引一覧 - デスクトップで改善されたレイアウト */}
                ...
                <div className="space-y-6" id="transactions">
                  ...
                  <ul className="space-y-4">
                    {eventData.transactions.map((tx) => (
                      <li 
                        key={tx.id} 
                        className="bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-lg transition-all cursor-pointer transform hover:-translate-y-1"
        ...
        <div className="lg:col-span-4 space-y-8">
          {/* メンバーセクション - デスクトップでは固定的なサイドバー */}
          <motion.div
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.2 }}
            className="bg-white border border-gray-200 rounded-2xl shadow-md p-6 sticky top-8"
          >
                ...
                {sortedMembers.map((member) => (
                  <div
                    ...
                    className={`p-3 rounded-xl border ${
                      ...
                      member.id !== 'organizer' ? 'cursor-pointer hover:shadow-md transform hover:-translate-y-1' : ''
                    } transition-all`}
                  >
                    <div className="text-center">
            ...
            {/* 精算結果セクション - デスクトップではサイドバーに追加 */}
            {eventData.transactions && eventData.transactions.length >= 2 && (
              <div id="settlement-results" className="mt-8 pt-8 border-t border-gray-200">
